  ...
  return hub.api.findTransactionObjects({ addresses: addresses.map(address => address.address) })
    ...
    .then(txs => txs
      .filter(tx => state.transfers
        .findIndex(stateTx => (stateTx.hash === tx.hash || stateTx.bundle === tx.bundle) && stateTx.persistence) === -1
      )
    )
    ...
    .then(txs => txs
      .concat(
        state.transfers.filter(stateTx => stateTx.hash &&
          state.transfers.findIndex(_stateTx => _stateTx.bundle === stateTx.bundle && _stateTx.persistence) === -1 &&
    ...
    .then(txs => txs
      .filter(tx => state.transfers
        .findIndex(stateTx => stateTx.hash === tx.hash && stateTx.persistence === tx.persistence) === -1
      )
    )
}
